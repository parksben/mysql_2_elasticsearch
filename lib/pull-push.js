...
exports.pull = function(pool, table, sqlPhrase, filter_map, exception_handler, callback) {
  ...
  var json2bulk = function(originData, filter_map, exception_handler) {
    ...
    var pattForType_three = /\"(\d{4})(\.|\-)(\d\d?)((\.|\-)\d\d?)?\"/gi;
    resData = resData.replace(pattForType_three, function (item){
      var middleStr = resData = resData.replace(pattForType_three, '$1_$3_$4');
      var dateArr = middleStr.split('_');
      var resDateStr = dateArr[0];

      if (dateArr[1].length == 1) {
        dateArr[1] = '0' + dateArr[1];
      }
      resDateStr += '-' + dateArr[1];

      if (!!dateArr[2]) {
        dateArr[2] = dateArr[2].replace(/\.|\-/, '');
        if (dateArr[2].length == 1) {
          dateArr[2] = '0' + dateArr[2].replace(/\.|\-/, '');
        }
        resDateStr += '-' + dateArr[2];
      }
        
      return '"' + String(Date.parse(resDateStr)) + '"';
    });
